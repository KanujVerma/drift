...
**Amendment, issue 111 (gap G6 of the strategy-surface contract inventory, #113).** The engine no longer stages the intent a strategy returns as returned. In both decision lanes, the answer of `decide` or `decide_exploratory` is revalidated before staging, as issue 78 revalidates every engine input. It must be exactly a `StrategyDecisionIntentV1`: not a subclass, which can declare fields the frozen schema does not have, and not any other object, `None` and dicts included. It must pass that type's strict validation. And it must already be the intent that validation rebuilds, so an unsorted target set, or state set on the instance past its frozen guard, is refused rather than silently repaired. Each refusal raises `StrategyIntentRejectedError`, so the run halts `REJECTED` before any fill with a named cause in `rejection_reason`, exactly as a staging refusal does. Staging reads the rebuilt intent, and a genuine intent's trace and result hashes are unchanged. A refused return with no canonical form (a forged naive `decision_time`, an arbitrary object) is traced with `intent_hash` over `{"uncanonical_return_type": "<module>.<qualname>"}` of its type. This covers only what a strategy returns: an exception raised inside `decide` or `decide_exploratory`, a `ValidationError` included, still propagates, and the M0 run records `FAILED`. Whether that should be `REJECTED` is decision D6 of #113, not decided here.
...
- *Session windows.* The Phase 1 pass of each clock session owns every effect and entitlement dated after the previous clock session, up to and including its own date. A split, stock dividend, spin-off, acquisition, or liquidation dated on a weekend or a did-not-open day is therefore applied at the next pre-open, where nothing has traded since the prior close. It is never dropped, and never applied twice. The first clock session owns only its own date: the opening book is taken to reflect every earlier effect, including any entitlement it carries as a pending claim. A window spans several dates when the clock skips days, and the pass applies share actions by security and record, not by date. So share actions touching one security (as actor, or as a child or acquirer) on two or more dates of one window are `INDETERMINATE` for a book exposed to any security those actions touch, because their order is unproven. Exposure is judged against both the prior close's book and the book the pass leaves, so a disposal that empties the first, or a chain of conversions that reaches a security only in the second, is still caught. Within a window, a cash distribution's pre-action share count is the prior close's, and its post-action count is the count after that security's single share-action date.
