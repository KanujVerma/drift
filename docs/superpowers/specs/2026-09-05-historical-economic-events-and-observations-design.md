...
Date: 2026-09-05. Status: independently reviewed design. M1c is now implemented and accepted; the [completed M1c execution record](../plans/2026-09-05-m1c-corporate-actions-economic-outcomes.md) preserves its decomposition, bounded refinements and verification. M1d is implemented and accepted; its [completed execution record](../plans/2026-09-07-m1d-source-observations-sessions-normalization.md) preserves its bounded semantic refinements and 81-row acceptance matrix. Dated design statements below are historical; current lifecycle status belongs in the roadmap.

Canonical destination: `docs/superpowers/specs/2026-09-05-historical-economic-events-and-observations-design.md`.

Design baseline: `14bad1733222758ee3836568a10a90f2a16aeac3`, verified against Git. At that baseline M0, M1a and M1b were complete; neither M1c nor M1d was implemented. This specification remains a design, not an implementation plan, provider choice, or experiment authorization. The separately authorized M1c execution is recorded in the linked completed plan.
...
This reviewed document supersedes the former M1c portion and related unresolved design questions of `2026-09-02-m1b-m1c-historical-equity-semantics-design.md`. That umbrella remains historical context and M1b provenance. Current lifecycle status belongs in the roadmap: M1c and M1d are complete. M1d's accepted synthetic capability is source claims, sessions, orthogonal missingness, finite M1b composition, M1c mapping, source/split views, and fixture replay across 81 matrix rows. It establishes no provider acceptance, evaluator, backtester, broker, trading, returns, portfolio, network, dependency, or environment-closure capability. Checkpoint records verified documentation state; no Session Handoff was needed for the completed design task.
...
**Amendment, issue 71 (ruling Q18 of #62, option b; decisions D1 to D9 adopted 2026-09-24).** The "equivalent complete-coverage derivation" above now exists, for exploratory evidence only. `ClosedWorldSessionCoverageV1` (`src/drift/domain/session_closed_world.py`) is one self-hashed, provider-neutral reading of one retained calendar response. It binds the requested interval and the hash of the request declaration, the SHA-256 and size of the exact retained response bytes, the measured origin record over those bytes, the returned date set (sorted, unique and inside the requested interval), a `ClosedWorldCompletenessAssertionV1`, the covered interval, the exact snapshot instant, the closed-row template (timezone, source methodology, availability channel), `evidence_grade: Literal["exploratory"]`, the static `derivation_algorithm_hash` of the rule, and the `m1d-evidence-v1` identity as `implementation_hash`. The covered interval is the bracketed hull `[min returned, max returned]`, never the requested interval (D2-b), so a truncated head or tail can only leave dates unknown. The rule (`src/drift/markets/session_closed_world.py`): a returned date is scheduled open, as schedule evidence only and never a realized session (D7-a); a covered date that was not returned is an evidenced non-trading date, but only under a positive assertion (every structural check held and the basis is provider-published or provider-partially-published); every other date, including a date on which two records of one source disagree, is INDETERMINATE and never closed. Records are unioned only within one source, venue and grade. The derivation is materialized as explicit `state="closed"` `ScheduledSessionVersionV1` rows bound to the record's canonical bytes (D4-A), so the existing generator, dense coverage walk and supporting-artifact closure consume them unchanged and the record enters `m1d_context_hash`. No pinned M1d producer or validator changed. Those validators cannot tell a derived closed row from a provider-printed one, so `verify_closed_world_session_coverage` re-verifies every record a context carries (retained bytes, request binding, origin, policy statement, identity, and exact equality of the covered schedule rows with the returned dates and the expansion), and every consumer that needs a definite session answer calls it. One property of the pinned generator is recorded here: a closed row carries no boundary offsets, so under a policy whose methodology set is nonempty a query on a closed date generates `indeterminate` (`generation_policy_methodology_set_mismatch`). That fails closed, and it is why consumers prove density through the record rather than by querying the generator on closed dates. The record module joins, and the derivation module is a seed of, `m1d-evidence-v1` (D5-a); the M1d freeze moves forward by `m1d-v8`. The "Missing date in unproven calendar list" row of section 12 is unchanged: a date outside every record's hull, or under a non-positive assertion, is still unknown, not closed.
