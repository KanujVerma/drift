...
- `PortfolioStateV2` adds `applied_effect_ids`, canonically sorted and unique, and requires every indeterminacy cause of every holding to be one of them. All V1 cash, claim, mark and NAV invariants hold through one shared validation function. A mark prices quantities only and never reads a basis status, so NAV stays `COMPLETE` over an indeterminate basis.
