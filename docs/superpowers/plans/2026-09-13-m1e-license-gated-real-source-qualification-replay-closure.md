...
> **Note (2026-09-25, #134).** Since #63 stage 2 (PR #132, merge `cde7728`), the M1c identities are semantic attestations too. `economic_validator_implementation_hash()` returns the `m1c-source-validation-v1` attestation, and the M1c selection, projection and composition identities bind `m1c-evidence-v1`. Both closures are declared in `src/drift/domain/semantic_attestation.py`, and each is a strict subset of the matching M1d closure. `economic_implementation_hash()` and `drift_source_inventory_hash()` remain whole-tree build and repository provenance only. The M1c freeze advanced by the chained link `m1c-v3`, and v2 remains the archive authority for `aecee94`. M1c evidence retained before stage 2 carries whole-tree identities and refuses re-validation by design; repository history replays from archives. See "Amendment, issue 63 (stage 2 of the Q6 ruling)" in the M2 spec. When Task 8 resumes, it must re-read the M1c identities from `main` as well.
