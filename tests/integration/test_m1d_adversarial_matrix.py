...
def test_current_v3_fixture_hash_index_is_exact() -> None:
    ...
    assert parsed["fixture_version"] == "m1d/v3"
...
def test_v3_generator_reproduces_exact_bytes_and_refuses_prior_versions(
    tmp_path: Path,
) -> None:
    generated = tmp_path / "v3"
    write_fixture_v3(generated)
    ...
    with pytest.raises(FileExistsError, match="v1 fixture cannot be overwritten"):
        write_fixture_v3(V1_FIXTURE_ROOT)
    with pytest.raises(FileExistsError, match="v2 fixture cannot be overwritten"):
        write_fixture_v3(V2_FIXTURE_ROOT)


def test_v3_generator_exclusively_reserves_concurrent_target(
    tmp_path: Path,
) -> None:
    target = tmp_path / "v3"
    start = Barrier(3)
    ...
    def writer(name: str) -> None:
        start.wait()
        try:
            write_fixture_v3(target, payload_factory=payload_factory)
    ...
    assert not (tmp_path / ".v3.generation.lock").exists()
    with pytest.raises(FileExistsError, match="cannot be overwritten"):
        write_fixture_v3(target, payload_factory=payload_factory)


def test_current_expected_decision_and_outcome_bytes_replay() -> None:
    fixture = load_m1d_fixture_v3()
...
def test_fixture_only_replay_needs_no_unit_harness_or_generator_import() -> None:
    ...
    program = """
import sys
from m1d_fixture_support import load_m1d_fixture_v3, materialize_result
loaded = load_m1d_fixture_v3()
