...
- *Continuing liquidation.* A liquidation whose claim is `continuing` is a partial liquidating distribution. Every share, its basis, and its target survive, and it is entitled, ordered after same-session share actions, and reconciled exactly as a cash distribution under section 12.3 (its `ex` date governs). Changing no share count, it is not a share action for the issue 82 rule on windows that span several dates.
- *Unproven claim.* A `converted` or `unknown` claim status leaves it unknown whether any share survives, so it is `INDETERMINATE` for an exposed book. The same holds for the claim status M1c composes across all of an outcome's effects: when that is `unknown`, for example two liquidations at one instant with conflicting statuses or a continuing effect after an extinguishing one, a liquidation on either path, and a cash, stock or mixed acquisition, is `INDETERMINATE` for an exposed book, even though each effect reads cleanly alone.
