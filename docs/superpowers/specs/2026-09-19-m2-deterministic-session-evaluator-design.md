...
**Amendment, issue 81.** Translation applies to every share-mutating action, not only splits; a target left in pre-action units made the next open trade shares no decision asked for, and the run still read `COMPLETE`. A restated target moves through the exact function its holding moves through: the same ratio, `ratio_meaning`, `FractionTreatmentV1`, and tie-breaking rule, with the rule above for a fraction the treatment cannot resolve (any aggregate-sale residual is dropped, since a target is owed no cash in lieu). A `STOCK_DIVIDEND` multiplies the target by $1 + n/d$, so a split booked as a stock dividend translates identically. A `SPINOFF` leaves the parent target unchanged, because parent shares are unchanged, and adds the whole child shares the book received to the child target, so the child is held rather than traded. This happens only when the parent carries a staged target, since otherwise no decision is staged. A `STOCK_ACQUISITION` or `MIXED_ACQUISITION` maps the predecessor target onto the acquirer at the ratio, adds it to any acquirer target, and sets the predecessor target to 0. A `CASH_ACQUISITION` sets the target to 0 once the ended claim is proven. Each of these also applies to a staged target with no holding behind it, as the split rule already did. A spin-off child or acquirer that is already held but carries no staged target, while the source target is staged, is `INDETERMINATE`: treating its missing target as 0 would sell a holding no decision named. `LIQUIDATION` is not yet translated, and its claim-status handling is open in issue 83.
