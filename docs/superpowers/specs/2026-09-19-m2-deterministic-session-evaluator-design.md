...
- *Continuing liquidation.* A liquidation whose claim is `continuing` is a partial liquidating distribution. Every share, its basis, and its target survive, and it is entitled, ordered after the pass's share actions, and reconciled exactly as a cash distribution under section 12.3 (its `ex` date governs). Changing no share count, it is not a share action for the issue 82 rule on windows that span several dates.
