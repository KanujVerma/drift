...
**Status:** Complete and historical. M1c was implemented under the separate 2026-09-05 authorization from planning checkpoint `8bc8bbed3abe0ce977445184e4aa495a6a89dd03`; runtime and fixture acceptance concludes at `8f2621d20c2b010ffc105b2e1aded65acaa9c091`. All eight tasks are complete with independent review and repository verification. Documentation publication uses the subject `docs: complete M1c economic fact milestone`. Its actual commit hash and post-commit Checkpoint result are verified and reported separately by the controller; this document does not self-attest that later check. The original planning-only record below remains historical. M1d is designed, unplanned and unimplemented; there is no executable M1d plan or authority to begin it.
...
- This was the single active executable M1c plan during implementation and is now the completed execution record. Its original authoring request stopped after planning; the subsequent 2026-09-05 implementation request authorized the reviewed M1c slices and commits. The M1d boundary still applies.
...
| `tests/integration/test_m1c_action_matrix.py` | Isolated 27-scenario economic acceptance matrix | 7 |
...
Task 7 execution decomposition: retain the required fixture loader/lineage tests in `test_m1c_economic_history.py`, and put the 27 named isolated scenario cases in additive `tests/integration/test_m1c_action_matrix.py`. The loader needs no shared-helper edits, so separate owners can work without overlapping files. This is a test-file responsibility split only; all source, fixture immutability, real-validation and acceptance requirements remain unchanged. Additional pinned identity and supporting-evidence files belong inside the new fixture directory; the loader must not regenerate its historical identity inputs.
...
- [x] Write one end-to-end RED with a real fixture and exact expected lineage:
...
- [x] Full gate, immutable fixture hash review, and fresh adversarial reviewer covering all matrix rows. Proposed boundary: `test: prove M1c economic histories and adversarial boundaries`.
...
Final-review hardening at `4d54d7e553beba8cdd5413ea1181e7efac7a236b`: the independent whole-milestone Astra review and controller reproductions found five bounded runtime gaps: authentic FAIL decisions accepted as authority, caller mappings snapshotted after value verification, before-window facts bypassing finite K/V, pre-window payments emitted as in-window delivery groups, and indeterminate actual evidence omitted from claim/residual uncertainty checks. Task 8 therefore includes repairs confined to the three existing M1c `economic_validation.py`, `economic_selection.py` and `economic_outcomes.py` modules, focused `tests/unit/test_m1c_final_hardening.py`, the already deferred M1c selection-test reason assertion, and versioned fixture replay tests. No M0/M1a/M1b validator or wire contract changes are authorized. The accepted v1 fixture remains byte-identical; corrected implementation bindings belong in new `tests/fixtures/m1c/v2/liquidation/` evidence. The fixture loader must distinguish archived code bindings from current replay rather than relabel old validation as current. These repairs enforce the existing contracts, not a new milestone or source family. Completion remains pending RED/GREEN, independent re-review, full gates and canonical status updates.

The reviewed hardening also separates occurrence-comparison evidence from window emission. Compare causally selected, owner/security-admitted reports of a positively evidenced same occurrence before emission, so neither window edge can hide a known conflicting source date or amount. Outside-window same-occurrence reports may constrain equality but cannot emit outside-window deliveries. Different occurrence IDs, non-owner sources, superseded revisions and causally unavailable reports cannot contaminate a group. Likewise, an indeterminate effect definitely earlier than a later terminal effect must not erase established precedence, while terminal resurrection and genuinely overlapping or unknown chronology remain fail-closed. These refinements follow reproduced repair counterexamples and preserve existing exact source identities, not a new reconciliation mechanism.

- [x] Reconcile actual HEAD and every accepted slice/review with this plan. Leave unresolved slices unchecked; mark completion only with evidence. Read current canonical documents before editing status.
- [x] Run the full gate and baseline compatibility tests through their existing entry points:
...
- [x] Get independent final acceptance covering temporal roles, duplicate/missing settlements, exact replay and M1d scope. No unresolved Important/Critical issue may be represented as complete. Re-run only affected tests and the full gate after material corrections.
- [x] Update canonical status to M1c complete, M1d designed/unplanned/unimplemented, with actual verification counts and accepted commit/review evidence. Preserve the fact that this plan's original authoring was planning-only. Do not claim evaluator/backtest readiness. Proposed documentation boundary: `docs: complete M1c economic fact milestone`.
- [x] Commit reviewed implementation paths under the separate authorization and run Checkpoint: verified at `8f2621d20c2b010ffc105b2e1aded65acaa9c091`. Documentation publication and its post-commit Checkpoint are external finalization actions; their actual hash/result belong in the controller's final report, not self-referentially in this commit. Do not create a new Session Handoff when execution completes cleanly.
...
Task 7 adds a typed verified-byte loader and 37 pinned liquidation fixture files, plus 27 named isolated action/attack scenarios. Economic source facts use the new fixture's 2020 history and uid(9000) namespace. Both settlement source versions and initial/corrected coverage versions coexist. A/B are real partitions under one source and one owner, not a second provider. Named methodology bytes are actual referenced profiles. The literal non-recursive expected-index digest is `90eb1764cb1f2ee25b84b2c138340e55d51fbe880237c6b59d53ecdab8bbac10`; no loader regenerates identity evidence, policy, or expected hashes. Accepted v1 bytes must not be rewritten later.

Two Terra implementation slices enabled separate file ownership, but independent Sol reviews found meaningful semantic gaps despite passing initial tests: unrelated histories combined, missing premises and exact oracles, self-labelled unsupported cases, pre-termination timing, absent fixture corrections and decorative named inputs. The controller reproduced important premises and escalated both bounded repairs to fresh Sol. Final independent reviewers approved all 27 matrix rows and fixture spec/quality, with no remaining findings. Known unsupported source facts remain evidence; the paired fixed/conditional validator rejection prevents false fixed admission. Real M1b termination replay precedes the continuing M1c claim. No source, shared helper, dependency or prior fixture/test changed in Task 7.

The initial fixture newline and descriptor-deserialization errors were construction failures, not economic RED. Existing-runtime scenario success is recorded honestly as characterization. The correction repair produced a genuine pre-change behavioral RED (one retained installment version instead of two), followed by GREEN. Final tests assert exact two-group multiplicity, USD 5 corroboration, the retained USD 4 to USD 5 revision transition, exact contributing hashes, supersession, cutoff selection, residual uncertainty, and tamper rejection. Earlier V over a later full inventory selects old facts but does not fabricate complete historical composition. Five obsolete unaccepted generated support files were removed before first acceptance; old accepted fixtures are untouched.

Focused acceptance: 27 matrix and 4 fixture tests. Controller final gate: 1,053 tests passed (28.66s), Ruff lint, format check (115 files), mypy (85 files), package build and diff check passed; the combined reviewed file hashes matched. Independent fixture review also checked exact generator/byte equality and verified old snapshot preservation. Task 8 final whole-milestone review, compatibility gates and canonical completion are next after the planned `test: prove M1c economic histories and adversarial boundaries` commit and Checkpoint. M1d remains designed only, without executable plan or implementation authority.

### Task 8 runtime hardening acceptance

A fresh Astra reviewed the complete seven-commit milestone, all eight runtime modules, the 27-case matrix, the immutable loader and actual fixture bytes. Five Important groups were reproduced independently by the controller: genuine failed validations admitted as authority, mutable Mapping substitution after validation, a before-window finite-cutoff bypass, out-of-window delivery emission, and indeterminate evidence omitted from claim/closure constraints. Sol implemented one bounded hardening slice with reviewed follow-ups. The final Astra review approves spec compliance, quality and runtime behavior with no unresolved Important, Critical or Minor finding; the deferred identity-reason assertion is now present.

The first repair RED was 17 behavioral failures with 14 controls passing before source edits. Follow-ups used explicit failing precedence, symmetric window-conflict and valid cancelled/unknown-effect cases before their fixes. Final behavior requires real PASS in both economic and identity admission, snapshots each supplied Mapping before checking/returning it, enforces K/V even for historical context facts, compares causally selected same-occurrence evidence before either window filter, emits only in-window deliveries, and preserves both relevant uncertainty and definite precedence. Cancellation/unknown variants remain facts without being treated as occurred claim assertions. Independent known payments survive missing or uncertain dependencies. No amount aggregation, evaluator, new source family, old contract or dependency change was needed.

The whole-package fingerprint remains separate from semantic-rule identity. Accepted v1 bytes remain unchanged and semantic replay is explicitly tied to implementation commit `4d54d7e553beba8cdd5413ea1181e7efac7a236b`. Current v2 retains identical economic, methodology, identity and support bytes; only manifests, policy and expected-hash index bind the corrected implementation. Final installed implementation identity is `00b872542bf905699b2065a4178f9f21b0ca7f74ccc38d9e9c99888ebd26c0a8`; v2 index is `ed3a9cf19c502580e170484a3e15853e8a05d83f7b4ac24b8af6b333c48686c4`. Old proofs are not relabelled as current validation. The independent reviewer checked all 43 repair-file hashes and both fixture versions, including the metadata-only differences.

Controller final runtime gate: 1,093 tests passed (35.40s), Ruff lint and format (116 files), mypy (86 checked files), build and diff checks passed. The required compatibility entry points passed all 366 tests (3.85s). No protected pre-M1c source/test/fixture/dependency path changed, and the forbidden runtime search had no matches. Unrelated metadata remains excluded: three existing `.DS_Store` hashes changed outside the scoped edits and two additional `.DS_Store` files appeared; their origin is not established, and no restoration or staging was attempted. The historical M1b handoff remains unchanged and untracked. The intended hardening subject is `fix: harden M1c evidence and temporal authority`. Canonical lifecycle edits and the final documentation checkpoint still follow; M1d remains unplanned and unimplemented.

### Task 8 canonical completion

This is the completed implementation record prepared for documentation publication. The controller must still verify and report the publication commit and its post-commit Checkpoint after they occur. The existing implementation Checkpoint below is completed evidence, not an attestation of that later publication check.

M1c remains one coherent historical-fact milestone. Runtime acceptance is committed at `8f2621d20c2b010ffc105b2e1aded65acaa9c091`; its post-commit Checkpoint passed all 1,093 tests (33.12s), Ruff lint/format, mypy, build and diff checks, with a clean tracked tree and index. The earlier final source gate passed 1,093 tests and all checks, and the separately required compatibility suite passed 366. Final independent Astra review resolved every finding and verified the current v2 bindings while preserving v1. The canonical README, overview, roadmap, AGENTS, ADR 0009 status and specification lifecycle now agree on M1c completion. Dated planning/design history is retained. No new Session Handoff is needed for this canonical completion; the existing M1b handoff remains unrelated and untracked.

Routing was capability-based, not evidence of correctness: Terra handled initial well-specified primitive/coverage and integration-fixture work; Sol handled difficult family semantics, validation, temporal selection, composition, repairs and adversarial reviews; a fresh Astra performed the whole-milestone review and scoped final acceptance. Initial Terra integration work required meaningful Sol rework after passing tests proved too weak. Sol repairs also required independent follow-ups for precedence, window comparison and payload variants. No unsupported claim of measured dollar savings is made. The controller verified material findings against live code and runnable counterexamples. Bounded mechanical edits did not require a separate Luna worker. Model-routing policy was not added to AGENTS or the product runtime.

Preserved boundaries: no old M0/M1a/M1b source, schema, serializer, fixture, event or replay contract was changed; no dependency was added. There is no M1d implementation or executable plan, provider acquisition/evaluation, network client, broker/credential/agent runtime, price/session/observation/normalization, holdings conversion, portfolio accounting, returns, reinvestment, evaluator, backtester or trading capability. Hashes and frozen models remain integrity mechanisms, not process isolation, source truth, or retained environment closure. Those later experiment and integration gates have not been claimed complete.

Lessons reserved for later M1d planning, not an implementation plan: keep source facts, causal availability, applicability, materialization and interpretation distinct; test both window edges and all payload variants; compare known same-occurrence evidence before projection filters; preserve exact-value snapshots and genuine validation success; retain immutable source bytes when interpreter bindings change. Whole-package fingerprint churn remains an explicit conservative cost, not a semantic version. M1d must eventually join action facts, source observations and pinned sessions without treating known data as eligible or executable trading authority. No material unresolved architectural question blocks this M1c result; user review and new authorization are required before any next milestone work.
