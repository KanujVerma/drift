...
**Amendment, issue 117.** A split, a reverse split, a stock dividend and a spin-off act on a claim that continues, so M1c claim evidence that the claim ended contradicts them.
- *Own status.* One whose own `claim_status` is `extinguished` or `converted` is contradictory evidence.
- *Composed status.* So is one on a claim M1c composes as `extinguished` or `converted` when no effect of the same outcome whose own status ends the claim definitely follows it. M1c composes the status of the claim's latest effect, so the claim had then ended by the action. An end that definitely follows the action, such as a split and then a cash acquisition in a later window, is the history of a live claim: the split applies in its own window and the acquisition in its own.
- *Exposure.* Either is `INDETERMINATE` for a book exposed to the acted security (a holding or a positive staged target) in the window the action's effective date falls in, judged against both the prior close's book and the book the pass leaves, like the composed-unknown rule above. The halt names the security, the action and the status. The prior close's book matters because a reverse split may restate a staged buy as 0. For an unexposed book the evidence is unrelated, and the action is a no-op.
