...
**Amendment, issue 87 (owner ruling A; causality finding F4 of the #8 final acceptance).** The reconstructed decision context was satisfied when any cohort member had a reconstruction for the decision session. A member missing its bar therefore reached the strategy with history ending at an earlier session, read as current: a lookback indexed by position spanned misaligned sessions, a target for the member filled at the next open, and the run was COMPLETE. The context now guarantees currency per member. Each reconstructed decision selects its history as before (issue 66) and then requires every cohort member with at least one reconstruction in that history to have one for the decision session itself. If any member lacks it, Phase 5 halts `INDETERMINATE` (`indeterminate_valuation`, phase `post_close_decision`) before the context is built and before the strategy is asked, so nothing is staged at that close and nothing fills from it. The cause names the session and every lacking member in canonical security order, with the last session its history reaches. The halt applies whether or not the strategy would trade the member, because the context is incomplete either way. A member whose reconstructions stop therefore halts the first decision after its last bar. A member with no reconstructed history at the decision cutoff follows the existing rules. It has no view in the context and stays in `admitted_cohort`, so a target for it stages, and it trades only if the execution session carries its reconstructed open; otherwise Phase 2 halts `INDETERMINATE` on the missing price. A member whose first reconstruction comes later joins the context at that session. When no member has a reconstruction for the decision session, the existing halt for missing decision evidence still applies, and a held member missing its bar still halts earlier, at the Phase 4 mark. A context in which every member with history is current is unchanged, byte for byte. Known bound: the check runs only where a decision is taken, so a member's missing bar on a warmup session is not refused and appears as a gap in a later decision's history.
